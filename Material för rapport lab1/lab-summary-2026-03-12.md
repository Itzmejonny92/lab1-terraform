...
Jag verifierade aven detta genom att kora `terraform plan` och `terraform apply` pa nytt. `plan` fungerade, och jag kunde samtidigt konstatera att state lock inte langre var den kvarvarande blockern. `apply` korde aterigen mot GCP och misslyckades med samma `403`-fel, vilket bekraktade att blockern fortfarande ar IAM-behorigheter och inte ett tillfalligt verktygsfel eller ett problem i Terraform-koden.
